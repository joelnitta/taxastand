...
# Use docker to run taxon-tools
options(ts_docker = TRUE)
```
# taxastand
...
`taxastand` depends on [taxon-tools](https://github.com/camwebb/taxon-tools) for taxonomic name matching. The two programs included in `taxon-tools`, `parsenames` and `matchnames`, must be installed and on the user's `PATH`.

## Docker image

A docker image is available to run `taxastand`: [`joelnitta/taxastand:latest`](https://hub.docker.com/r/joelnitta/taxastand).

The [Dockerfile lives here](https://github.com/joelnitta/taxastand-docker).
