...
For example, the name [*Hymenophyllum wilsonii*](https://www.tropicos.org/name/Search?name=Hymenophyllum%20wilsonii) corresponds to two different scientific names with different authors, *Hymenophyllum wilsonii* Fée and *Hymenophyllum wilsonii* Hook.

We can see this by querying the name without an author:
...
natalense_res <- ts_resolve_names("Hymenophyllum natalense", filmy_taxonomy)
head(natalense_res)
dim(natalense_res)
```

In this case, name resolution using the default settings produced `r nrow(natalense_res)` possible answers! That is obviously far too many. The trouble is that our query lacked an author, so it fuzzily matched many similar names. Since the query has no author anyway, let's allow matching names without the author (`match_no_auth = TRUE`):
...
Name resolution workflows typically involve tweaking the matching arguments (such as `match_no_auth`, `match_canon`, and `max_dist`) to resolve as many names as possible automatically, followed by some amount of manual editing of the names that remain unresolved.
