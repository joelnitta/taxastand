...
This vignette explains the three basic steps of the taxonomic name resolution workflow, which consists of:
...
In this case, name resolution using the default settings produced `nrow(t_bifid_res)` possible answers! That is obviously far too many. Let's try to adjust the arguments and see if we can reduce the output:
...
This vignette illustrated the typical steps involved in name resolution with `taxastand` on some trivial examples. In another vignette, I will provide a more realistic example with a bigger dataset.
